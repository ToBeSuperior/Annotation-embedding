...
class CompositionDataset(Dataset):
    ...
    def __init__(
        ...
        elif self.phase == 'seen_val':
            inter = set(self.train_pairs) & set(self.val_pairs)
            self.data = [[img, attr, obj] for img, attr, obj in self.val_data if (attr, obj) in inter]
            self.phase = 'val'
        elif self.phase == 'unseen_val':
            inter = set(self.train_pairs) & set(self.val_pairs)
            self.data = [[img, attr, obj] for img, attr, obj in self.val_data if (attr, obj) not in inter]
            self.phase = 'val'
        elif self.phase == 'test':
            self.data = self.test_data
        elif self.phase == 'seen_test':
            inter = set(self.train_data) & set(self.test_data)
            self.data = [[img, attr, obj] for img, attr, obj in self.test_data if (attr, obj) in inter]
            self.phase = 'test'
        elif self.phase == 'unseen_test':
            inter = set(self.train_data) & set(self.test_data)
            self.data = [[img, attr, obj] for img, attr, obj in self.test_data if (attr, obj) not in inter]
            self.phase = 'test'
        elif self.phase == 'all':
    ...
    def get_split_info(self):
        ...
        # some images in mit-states have special unicode in their file name
        if self.root == 'DATA_ROOT/mit-states' :
            exc_num = []
            for i, sample in enumerate(data):
                if not os.path.exists('DATA_ROOT/mit-states/images/'+ sample['image']):
                    exc_num.append(i)
            for i, index in enumerate(exc_num):
                del data[index-i]
