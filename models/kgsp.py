...
class KGSP(nn.Module):
    ...
    def train_forward(self, x):
        ...
        lema_loss = 0
        if self.args.lema:
            attr_pred_normed = F.softmax(attr_pred, dim=1)
            obj_pred_normed = F.softmax(obj_pred, dim=1)
            
            attr_pos_score = attr_pred_normed[torch.arange(len(attrs)), attrs[torch.arange(len(attrs))]]
            obj_pos_score = obj_pred_normed[torch.arange(len(objs)), objs[torch.arange(len(objs))]]
            
            attrs_list = [[x] for x in attrs.tolist()]
            objs_list = [[x] for x in objs.tolist()]
            
            attr_negs_score = torch.stack([attr_pred_normed[idx][torch.tensor(list(set(range(attr_pred_normed.size(1))) - set(exclude)))] for idx, exclude in enumerate(attrs_list)])
            obj_negs_score = torch.stack([obj_pred_normed[idx][torch.tensor(list(set(range(obj_pred_normed.size(1))) - set(exclude)))] for idx, exclude in enumerate(objs_list)])
        ...
        loss =  attr_loss + obj_loss + lema_loss
        if self.args.gumbel==True and self.args.partial == True:
            loss = loss+weak_loss
